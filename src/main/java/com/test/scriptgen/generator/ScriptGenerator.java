...
public class ScriptGenerator {
	...
	// Internal variables
	private ClassLoaderResourceAccessor classLoaderAccessor;

	// Input
	/** Name of the input file. Should match a file name that can be found on the classpath. */
	private String inputFile;
	/** The output directory in which generated SQL is stored. If empty, the resulting SQL will be printed to the console. */
	private File outputDirectory;
	/** String array of the dialects for which SQL is generated. Filled with db2, oracle, and mssql by default. */
	private String[] dialects;
	/** List of the phases for which SQL is generated. Filled with every phase except for migration phases by default. */
	...
	private boolean handleCommandlineArguments(String[] args) {
		...
		for (String arg : args) {
			...
			throw new IllegalArgumentException("Unknown argument: " + arg + ". Only '-i', '-o', '-dbms', and '-phases' are allowed.");
		}

		// If no values were passed, set the default values.
		// Set the input file.
		if (StringUtils.isEmpty(inputFile)) {
			inputFile = "QSD-42470.xml";
		}
	...
	/**
	 * Finishes initialization by checking this objects instance variables and initializing the {@link #classLoaderAccessor}.
	 */
	public boolean finishInitalization() {
		...
		for (String phase : phases) {
			...
			if (p < 0 || p > Phase.ORDER_OF_LAST_PHASE) {
				System.out.println("The phases should be larger than 0 and smaller than " + String.valueOf(Phase.ORDER_OF_LAST_PHASE) + ".");
				return false;
			}
		}

		// Initialize the class loader for liquibase.
		classLoaderAccessor = new ClassLoaderResourceAccessor();

		return true;
	}

	/**
	 * Loops over all phases and database dialects that were set during initialization,
	 * and generates sql scripts for them using the given changelog file.
	 * @throws LiquibaseException when liquibase fails to initialize.
	 * @throws IOException when the output files could not be created / saved / read.
	 */
	public boolean generateScripts() throws LiquibaseException, IOException {
	...
	/**
	 * Build an instance of liquibase, which holds an offline connection to a database specified by the dialect.
	 */
	private Liquibase buildLiquibase(String dialect) throws LiquibaseException {
		...
		// for a specific database, while still outputting insert statements for the DatabaseChangelog table.
		String url = "offline:" + dialect + "?outputLiquibaseSql=true";

		Liquibase liquibase = new Liquibase(
				inputFile,
				classLoaderAccessor,
				new OfflineConnection(url, classLoaderAccessor)
		);
	...
	/**
	 * Determines which writer to use for the output. This is a writer to the console if the output directory is empty,
	 * otherwise a writer for an output file.
	 */
	...
	/**
	 * Determines the filename of the file in which the sql for a certain phase and dialect are stored.
	 */
	private String determineFileName(String dialect, String phase) {
		return BASE_OUTPUT_FILE_NAME + "-" + dialect + "-" + Phase.determineNameByOrder(Integer.valueOf(phase)) + ".sql";
	}

	/**
	 * This class generates SQL scripts based on changelog files, for any given combination of phases and database dialects.
	 * USAGE: java com.keylane.ScriptGenerator [-i=inputFileName] [-o=outputDirectory] [-dbms=db2,oracle,mssql] [-phases=1,2,5,6,7]
	 * All parameters are optional. The input file should refer to a changelog that is available on the classpath.
	 * If the output directory is empty, the output will be logged to the console.
	 ...
	 *     <li>phases: 1, 2, 5, 6, 7 (sql is generated for all phases that conventionally use liquibase). </li>
	 * </ul>
	 * @param args
	 */
